...
import { AppError, check, normalizeSurvey, availability, publicSurvey, requireOpen, validId, validateAnswers, validateQuestionInterviews, newResponse, buildResults, toCsv } from './domain.js';
import { passwordMatches, signToken, verifyToken, isAdmin, adminCookie, requireSameOrigin } from './security.js';
import { createStore, getSurveys, getSurvey, getResponses } from './storage.js';
...
function attachQuestionInterviews(survey, response, tokens = {}) {
  ...
  response.questionInterviews = validateQuestionInterviews(survey, interviews, response.answers);
}
  ...
  return async function app(req, res) {
    try {
      ...
      if (route === '/api/admin/settings' && method === 'GET') return json(res, 200, {
        ...
        providers: [
          { id: 'openai', label: 'OpenAI', configured: Boolean(process.env.OPENAI_API_KEY), model: process.env.OPENAI_MODEL || 'gpt-5.4-nano' },
      ...
      if (route === '/api/admin/import' && method === 'POST') {
        ...
        const responses = data.responses.map(r => {
          ...
          clean.questionInterviews = validateQuestionInterviews(survey, r.questionInterviews, clean.answers);
          return clean;
        });
        ...
        for (const incoming of responses) {
          ...
          if (saved) {
            ...
            check(JSON.stringify([saved.attributes, saved.questions]) === JSON.stringify([imported.attributes, imported.questions]), '同じIDで設問定義が異なります。別の保存先へ復元してください。', 409);
          }
        }
      ...
      if (admin && !action && method === 'PUT') {
        ...
        if ((await getResponses(store, survey.id)).length) {
          ...
          const structure = s => JSON.stringify([s.attributes, s.questions].map(fields => fields.map(({ id, type, options, min, max, required, followUp }) => ({ id, type, options, min, max, required, followUp }))));
          check(structure(updated) === structure(survey), '回答があるアンケートの設問構造は変更できません。別のアンケートを作成してください。', 409);
        }
      ...
      if (!action && method === 'GET') return json(res, 200, { survey: publicSurvey(survey) });
        ...
        if (!body.token) {
          ...
          if (question) {
            ...
            if (question.followUp) {
              ...
              check(initial, 'AIと対話する前に理由を入力してください。');
            }
      ...
      if (action === 'responses' && method === 'POST') {
        ...
        const existing = (await getResponses(store, survey.id)).find(r => r.id === response.id);
        if (!existing) {
          requireOpen(await getSurvey(store, survey.id), clock());
