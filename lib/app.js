...
import { AppError, check, normalizeSurvey, availability, publicSurvey, requireOpen, validId, validateAnswers, validateQuestionInterviews, newResponse, buildResults, toCsv, responseLimit } from './domain.js';
  ...
  return async function app(req, res) {
    ...
    try {
      ...
      if (!action && method === 'GET') {
        ...
        if (result.availability === 'open' && (await getResponses(store, survey.id)).length >= responseLimit(survey)) result.availability = 'closed';
      ...
      if (action === 'interview' && method === 'POST') {
        ...
        check((await getResponses(store, survey.id)).length < responseLimit(survey), '回答数の上限に達したため、受付を終了しました。', 403);
