...
export function createStore() {
  ...
  check(['local', 'sheets', 'postgres'].includes(driver), '保存先の設定が不正です。', 503);
  check(!process.env.VERCEL || driver !== 'local', 'VercelではSheetsまたはPostgreSQL保存を指定してください。', 503);
...
export async function getSurvey(store, id) {
  if (store.getSurvey) return store.getSurvey(id);
  const survey = (await getSurveys(store)).find(s => s.id === id);
  check(survey, 'アンケートが見つかりません。', 404);
  return survey;
}
export async function countResponses(store, id) {
  return store.countResponses ? store.countResponses(id) : (await getResponses(store, id)).length;
}
