...
export function isInterviewRequired(field) {
  return Boolean(field.type === 'aiInterview' ? field.interviewRequired : field.followUp?.interviewRequired);
}
export function hasInterviewReply(turns) {
  return turns.some((turn, index) => turn.role === 'user' && turn.content.trim() && index > 0 && turns[index - 1].role === 'assistant');
}
...
export function normalizeSurvey(input) {
  ...
  const normalizeFields = (fields) => {
    ...
    return fields.map(field => {
      ...
      if (field.followUp) {
        ...
        followUp = { required: Boolean(field.followUp.required), maxTurns: depth, ...(field.followUp.interviewRequired === true ? { interviewRequired: true } : {}) };
      }
      return { id, label, type: field.type, required: Boolean(field.required), options, min, max, ...(field.type === 'aiInterview' ? { maxTurns, ...(field.interviewRequired === true ? { interviewRequired: true } : {}) } : {}), ...(followUp ? { followUp } : {}), aiContext: normalizeContext(field.aiContext) };
    });
  };
  ...
  check([...attributes, ...questions].reduce((n, f) => n + JSON.stringify(f.aiContext).length, 0) <= 60000, 'アンケート全体の補足情報は60000文字までです。');
...
export function publicSurvey(survey) {
  ...
  const publicFields = fields => fields.map(({ aiContext, ...field }) => field);
  return { id: survey.id, title: survey.title, description: survey.description, startsAt: survey.startsAt, endsAt: survey.endsAt, attributes: publicFields(survey.attributes), questions: publicFields(survey.questions), interview, showInitialReason: SHOW_INITIAL_REASON, availability: availability(survey) };
}
...
export function validateQuestionInterviews(survey, interviews = {}, answers = {}, reasons = {}) {
  ...
  for (const field of fields) {
    ...
    check(!isInterviewRequired(field) || interview, `${field.label}のAIインタビューに回答して終了してください。`);
    ...
    check(!isInterviewRequired(field) || hasInterviewReply(turns), `${field.label}のAIの質問に少なくとも1回回答してください。`);
