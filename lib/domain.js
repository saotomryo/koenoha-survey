...
export function normalizeSurvey(input) {
  ...
  return {
    id: input.id, title, description: string(input.description), status, spreadsheetId,
...
export function publicSurvey(survey) {
  ...
  return { id: survey.id, title: survey.title, description: survey.description, startsAt: survey.startsAt, endsAt: survey.endsAt, attributes: survey.attributes, questions: survey.questions, interview, availability: availability(survey) };
}
export function validateAnswers(survey, body) {
  ...
  for (const field of survey.questions.filter(f => f.followUp)) {
    ...
    check(!field.followUp.required || reason.trim(), `${field.label}の理由を入力してください。`);
...
export function validateQuestionInterviews(survey, interviews = {}, answers = {}) {
