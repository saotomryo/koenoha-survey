...
export function normalizeSurvey(input) {
  ...
  const normalizeFields = (fields) => {
    ...
    return fields.map(field => {
      ...
      return { id, label, type: field.type, required: Boolean(field.required), options, min, max, ...(field.type === 'aiInterview' ? { maxTurns } : {}), ...(followUp ? { followUp } : {}), aiContext: normalizeContext(field.aiContext) };
    });
  };
